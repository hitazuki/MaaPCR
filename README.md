...
- [x] 黎明界刷Boss初始 — 独立入口，支持难易度、三层/五层目标 Boss 多选与不符自动重开
...
- 「黎明界(基础框架)」用于正常自动化流程；「黎明界刷Boss初始」用于开局刷三层/五层 Boss，两者为不同任务入口。
- 刷 Boss 初始会沿用前置的目标公会与难易度选择流程；难易度会直接点击识别到的目标数字，若变更窗口仍停留则自动点击取消兜底。
- 三层目标 Boss 与五层目标 Boss 分开配置，均支持多选；识别到任一已勾选 Boss 即视为通过，否则会关闭详情、无报酬撤退并确认终了后重开。
- 使用刷 Boss 初始时，建议三层与五层至少各勾选一个目标 Boss。
