...
def update_pip(pip_config: dict) -> bool:
    """
    更新 pip 到最新版本
    """
    ...
    if _run_pip_command(cmd, f"更新 pip"):
        return True
    else:
...
def install_requirements(
    ...
    # 回退到在线安装
    mirror = get_available_mirror(pip_config) if pip_config else None

    if mirror:
        logger.info(f"使用源 {mirror} 安装依赖") 
        cmd = [
            sys.executable,
            "-m",
            "pip",
...
def check_and_install_dependencies():
    ...
    if enable_pip_install and (
        ...
        if install_requirements(pip_config=pip_config):
            if update_pip_config(current_version):
                logger.debug("成功更新pip配置中的interface版本信息")
            else:
                logger.warning("更新pip配置中的版本信息失败")
